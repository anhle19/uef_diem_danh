...
async function downloadStudentCards(studyClassId) {
    ...
    try {
        ...
        const response = await axios.post(
            `/api/tai-ve-danh-sach-the-hoc-vien/${studyClassId}`,
            null,
            { responseType: 'blob' }
        );
...
document.addEventListener('DOMContentLoaded', async (event) => {
    ...
    try {
        ...
        const response = await axios.get(`/api/lay-danh-sach-hoc-vien-theo-lop/${studyClassId}`)
        ...
        for (let i = 0; i < totalSheets; i++) {
            ...
            for (let j = 0; j < processingStudentsData.length; j++) {
                ...
                `
                 <div class="card student-card text-center">
                    <div class="card-top">
                        <img src="https://laitsolution.id.vn/student_pictures/${processingStudentsData[j].studentAvatar}" class="card-img-top" alt="Ảnh đại diện">
                        <div class="card-body">
